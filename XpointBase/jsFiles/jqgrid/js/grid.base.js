    ...
    */
    ...
    $.fn.jqGrid = function(p) {
        p = $.extend(true, {
            ...
            ExpandColumn: null,
            tree_root_level: 0,
            prmNames: { page: "page", rows: "rows", sort: "sidx", order: "sord", mid: "mid", currenturl: "currenturl", tmpid: "tmpid", isactivity: "isactivity", selview: "selview", reportquerytype: "reportquerytype" },
            sortclass: "grid_sort",
		...
		formatter = function(rowId, cellval, colpos, rwdat, _act) {
		    ...
		    if (typeof cm.formatter !== 'undefined') {
		        ...
		        if (cm.edittype == "richtextbox") {
		            v = $.jgrid.htmlDecode(cellVal(cellval));
		        } else {
		            v = cellVal(cellval);
		        }
		    }
		...
		addRowNum = function(pos, irow, pG, rN) {
			...
			prp = formatCol(pos, irow);
		    if (p.radiobutton) {
		        return "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum xp-CusTopNav\" " + prp + "><input type='radio' id='jqg_" + v + "' class='jqg_selectradio' name='jqg_selectradio' /></td>";
		    }
		    else {
		        return "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum xp-CusTopNav\" " + prp + ">" + v + "</td>";
		    }
		},
		...
		addJSONData = function(datap, t, rcnt) {
		    var data = datap.d;
		    ts.p.reccount = 0;
		    if (data) {
		        if (ts.p.treeANode === -1 && ts.p.scroll === false) {
		...
		updatepager = function(rn) {
		    ...
		    if (ts.p.pager) {
		        ...
		        if (ts.p.pginput === true) {
		            ...
		            var currentUrlofpage = location.href;
		            var paginationValue = ts.p.rowNum;


		            if (currentUrlofpage.indexOf('UserManager.aspx') > -1) {
		                $.ajax({
		                    type: "POST",
		                    url: "/_layouts/IImpact.Web/ADUserMngGridService.asmx/GetUsersCountOnSite",
		                    data: "{}",
		                    contentType: 'application/json; charset=utf-8',
		                    dataType: "JSON",
		                    success: function (result) {
		                        TotalUserCount = result.d;
		                        var count = result.d / paginationValue;
		                        var remainder = result.d % paginationValue;
		                        var roundOffValue = Math.trunc(count);
		                        if (remainder == 0) {
		                            $('#sp_1', ts.p.pager).html(roundOffValue);
		                        }
		                        else {
		                            $('#sp_1', ts.p.pager).html(roundOffValue + 1);
		                        }
		                    }
		                });
		            }
		            else if (currentUrlofpage.indexOf('ClientUserAdmin.aspx') > -1) {
		                $.ajax({
		                    type: "POST",
		                    url: "/_layouts/IImpact.Web/ADUserMngGridService.asmx/GetUsersCountPerPage",
		                    data: "{}",
		                    contentType: 'application/json; charset=utf-8',
		                    dataType: "JSON",
		                    success: function (result) {
		        ...
		        if (ts.p.viewrecords) {
		            ...
		            if (ts.p.reccount === 0)
		                ...
		                if (currenturl.indexOf('ClientUserAdmin.aspx') > -1) {
		                    $.ajax({
		                        type: "POST",
		                        url: "/_layouts/IImpact.Web/ADUserMngGridService.asmx/GetUsersCountPerPage",
		                        data: "{}",
		                        ...
		                        dataType: "JSON",
		                        success: function (result) {
		                            $(".ui-paging-info", ts.p.pager).html($.jgrid.format(ts.p.recordtext, from, to, result.d));
		                        }
		                    });
		                }
		                if (currenturl.indexOf('UserManager.aspx') > -1) {
		                    $.ajax({
		                        type: "POST",
		                        url: "/_layouts/IImpact.Web/ADUserMngGridService.asmx/GetUsersCountOnSite",
		                        data: "{}",
		...
		getColNames = function() {
            ...
            , populate = function() {
                ...
                if (!grid.hDiv.loading) {
                    ...
                    prm["colModel"] = getColNames();
                    prm["phasebased"] = ts.p.phasebased;
                    var sdata = { 'searchdata': $.extend(ts.p.postData, prm) };
                    ...
                    switch (dt) {
                        case "json":
                            $.ajax({ url: ts.p.url, contentType: "application/json; charset=utf-8", type: ts.p.mtype, dataType: "json", data: JSON.stringify(sdata), complete: function(JSON, st) {
                                if (st == "success") {
                                    addJSONData($.jgrid.parse(JSON.responseText), ts.grid.bDiv, rcnt); JSON = null; if (loadComplete) { loadComplete(); }
                                }
                            }, error: function(xhr, st, err) { endReq(); if (loadError) { loadError(xhr, st, err); } endReq(); }, beforeSend: function(xhr) { if (loadBeforeSend) { loadBeforeSend(xhr); } }
                            });
		...
		setPager = function() {
			...
			pginp = (ts.p.pginput === true) ? "<td>" + $.jgrid.format(ts.p.pgtext || "", "<input autocomplete='off' class='ui-pg-input' type='text' size='1' maxlength='7' value='0' role='textbox'/>", "<span id='sp_1'></span>") + "</td>" : "",
		    ...
		    $(ts.p.pager).addClass('ui-jqgrid-pager corner-bottom xp-NoBackground ')
		    ...
		    $('.ui-pg-selbox', "#" + pgcnt).bind('change', function() {
		        /*
		        * code by anirudh - if the number of rows to be shown is
		        * changed in pager  then reset the page value to 1st page
		        */
		        ts.p.page = 1;
		...
		td, ptr, tbody, imgs, iac = "", idc = "";
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                if (ts.p.colModel[i].headeralign != null) {
                    thead += "<div style='text-align:" + ts.p.colModel[i].headeralign + "' id='jqgh_" + ts.p.colModel[i].name + "' " + tdc + ">" + ts.p.colNames[i];
                }
                else {
		...
		hb = $("<div class='ui-jqgrid-hbox'></div>");
			...
			.addClass("ui-state-default xp-OuterPanel  ui-jqgrid-hdiv ui-corner-top  xp-BoxShadowTop ")
			.append(hb)
            ...
            if (ts.p.pager) {
                ...
                $(ts.p.pager).css({ 'background-color': 'transparent' }).css({ width: grid.width + "px" }).appendTo(eg).addClass('ui-jqgrid-pager xp-OuterPanel ui-corner-bottom xp-CusTopNav xp-BoxShadow');
            ...
            $(grid.bDiv)
			.append(this)
			.addClass("ui-jqgrid-bdiv xp-OuterPanel")
			.css({ height: ts.p.height + (isNaN(ts.p.height) ? "" : "px"), width: (grid.width) + "px" })
			.scroll(function(e) {
			    grid.scrollGrid();
			});
            if (ts.p.height != null && ts.p.height == "auto") {
                $(grid.bDiv).attr('style', 'overflow-y: hidden');
            }
...
//            var imghtml = "";
//            if (ts.p.imgurl != "") {
//                imghtml = "<img src='" + ts.p.imgurl + "' alt='image' class='" + ts.p.imgclass + "'></img>";
//            }
            $(grid.cDiv).append(arf).append("<div class=' xp-OverflowHidden'  style='float:left;width:auto'><div style='float:left;width:auto !important' class='xp-DivHeader'>" + ts.p.caption + "</div></div>")
		.addClass("ui-jqgrid-titlebar  ui-corner-tl ui-corner-tr ui-helper-clearfix");
            ...
            if (ts.p.toolbar[0]) {
                ...
                $(grid.uDiv).width(grid.width).addClass("ui-userdata ui-state-default xp-ClearBoth").attr("id", "t_" + this.id);
                }
            ...
            if (ts.p.footerrow) {
                ...
                hb = $("<div class='ui-jqgrid-hbox'></div>");
                $(grid.sDiv).addClass("ui-jqgrid-sdiv xp-OuterPanel").append(hb).insertAfter(grid.hDiv).width(grid.width);
                $(hb).append(tfoot);
            ...
            return this.each(function() {
                ...
                if (!$t.p.multiselect) {
                    if ($(pt).attr("class") !== "subgrid") {
                        ...
                        if ($t.p.rownumbers != null && $t.p.rownumbers && $t.p.radiobutton) {
                            $($($($(pt).children()).get(0)).children().get(0)).attr("checked", true);
                        }
                    }
                } else {
                    $t.p.selrow = selection;
        ...
        setGridWidth: function(nwidth, shrink) {
			...
			msw = $t.p.multiselectWidth, sgw = $t.p.subGridWidth, rnw = $t.p.rownumWidth, cl = $t.p.cellLayout, cr;
                ...
                if (shrink === true) {
                    ...
                    isSafari = $.browser.safari && parseFloat($.browser.version)<536.5 ? true : false; // Chrome < version 19
                    if (isSafari) { brd = 0; msw += cl; sgw += cl; rnw += cl; }
