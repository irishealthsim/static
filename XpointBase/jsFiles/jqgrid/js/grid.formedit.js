    ...
    $.fn.extend({
        ...
        editGridRow: function (rowid, p) {
            ...
            return this.each(function () {
                ...
                if ($("#" + IDs.themodal).html() != null) {
                    ...
                    viewModal("#" + IDs.themodal, { gbox: "#gbox_" + gID, jqm: p.jqModal, jqM: true, closeoverlay: closeovrl, modal: p.modal });
                    if (!closeovrl) {
                    ...
                    var flr, frm = ($.browser.msie && parseInt($.browser.version) === 7) ? $("<div name='FormPost' id='" + frmgr + "' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:" + dh + ";'></div>").data("disabled", false) :
                              $("<form name='FormPost' id='" + frmgr + "' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:" + dh + ";'></form>").data("disabled", false),
                    ...
                    // buttons at footer
                          ...
                          bS = "<a href='javascript:void(0)' id='sData' class='xpThemeButton xpPadding  ui-primarytabclr ui-corner-all'>" + p.bSubmit + "</a>",
                          bC = "<a href='javascript:void(0)' id='cData' class='cancelButton xpPadding ui-state-default ui-corner-all'>" + p.bCancel + "</a>";
                    var bt = "<table border='0' class='EditTable' id='" + frmtb + "_2'><tbody><tr id='Act_Buttons'><td class='EditButton ui-widget-content'>" + bS + "&nbsp;" + bC + "</td></tr>";
                    if (rp_ge.bottominfo) {
                    ...
                    //  if (p.funcAlt ===true) {
                    //       createResourceProject(p, "#gview_" + $t.p.id, $("#gview_" + $t.p.id)[0]);
                    //  }
                    //  else if (p.xbarChart == "true") {
                    //      createXbarChartModal(rowid, IDs, tms, p, "#gview_" + $t.p.id, $("#gview_" + $t.p.id)[0]);
                    //  }

                    if (p.isCAU === "True" && p.caption === "Add User") {
                        showErrorMessageforLicence();
                    }
                    else {
                    ...
                    if (rp_ge.checkOnSubmit || rp_ge.checkOnUpdate) {
                        bS = "<a href='javascript:void(0)' id='sNew' class='fm-button ui-primarytabclr ui-corner-all' style='z-index:1002'>" + p.bYes + "</a>";
                    ...
                    $(".FormElement", "#" + frmtb).each(function (i) {
                        switch ($(this).get(0).type) {
                            ...
                            case "textarea":
                                var $this = this;
                                var name = this.name;
                                // this is custom code. To take care rich textbox.
                                $.each($t.p.colModel, function (j) {
                                    var model = $t.p.colModel[j];
                                    if (model.name == name && model.edittype == "richtextbox") {
                                        $($this).SaveRichtextbox();
                                        return false;
                                    }
                                });
                                postdata[$this.name] = $.jgrid.htmlEncode($($this).val());
                ...
                function createData(rowid, obj, tb, maxcols) {
                    ...
                    $(obj.p.colModel).each(function (i) {
                        ...
                        if (hc == false && ind !== false) {
                            if (this.editrules && this.editrules.addonly == true) {
                                hc = true;
                            }
                        }
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && this.editable === true && nm !== 'rn') {
                            if (ind === false) {
                                ...
                                if (nm == obj.p.ExpandColumn && obj.p.treeGrid === true) {
                                    ...
                                    try {
                                        tmp = $.unformat($("td:eq(" + i + ")", obj.rows[ind]), { colModel: this }, i, false, obj.rows[ind].id);
                                    } catch (_) {
                                        tmp = $("td:eq(" + i + ")", obj.rows[ind]).html();
                                    }
                                }
                            }
                            //need to override the id because of bug with jqgrid
                            //Bug Ref: incase of multiple jqgrid having elements with same name
                            //it gives issues because of multiple elements with same id on the page
                            var opt = $.extend({}, this.editoptions || {}, { id: nm + obj.id, name: nm, imageEnabled: ($t.p.isImageEnabled == "False") ? false : true });
                            ...
                            var elem = elc;
                            if (this.edittype == "richtextbox") {
                                p.onClose = function (elem) {
                                    $(elem).RemoveRichText();
                                }
                            }
                            ...
                            $("td:eq(" + (cp - 2) + ")", trdata[0]).html(typeof frmopt.label === 'undefined' ? obj.p.colNames[i] : frmopt.label).append("<span style='color:red'>" + frmopt.elmsuffix + "</span>");
                    ...
                    if (cnt > 0) {
                        var idrow = $("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='" + (maxcols * 2 - 1) + "' class='DataTD'><input autocomplete='off' class='FormElement' id='id_g' type='text' name='id' value='" + rowid + "'/></td></tr>");
                        idrow[0].rp = cnt + 999;
                        $(tb).append(idrow);
                ...
                function postIt() {
                    $("#editcnt" + gID).append("<div id='relayAjaxLoading'><div class='xp-Width xp-TextAlignCenter xp-Height100 xp-relayAjaxLoader'><img src='/_layouts/Images/XPointBase/xp-loader.gif'style='z-index:999999999;text-align:center;'/></div></div>");
                    var copydata, ret = [true, "", ""], onCS = {}, islocal = false;
                    for (var key in postdata) {
                        ret = checkValues($.trim(postdata[key]), key, $t);
                        if (ret[0] == false) {
                    ...
                    islocal = $t.p.datatype == "local" ? true : false;
                    /*check for url only if the datatype is not local , incase of 
                    /*local datatype url validation is not required*/
                    if (!islocal) {
                        ...
                        if (ret[0] === false) {
                            ...
                            /*
                            * Code done by Anirudh - TO fix focus issues of the error messages
                            */
                            try {
                                $($("#FormError", "#" + frmtb).parents().get(1)).focus();
                            }
                            catch (Er)
                            { }
                            return;
                    ...
                    if (!p.processing) {
                        ...
                        for (i in postdata) {
                            fields[le] = i;
                            coldata[le] = postdata[i];
                            if (le == fields.length)
                                break;
                            le++;
                        }
                        gpost = new Object();
                        gpost.id = postdata.id == "_empty" ? "new" : postdata.id;
                        gpost.cells = coldata;
                        gpost.fields = fields;
                        gpost.mid = $t.p.mid;
                        gpost.listname = $t.p.listname;
                        gpost.extraEditParams = $t.p.extraEditParams;
                        if (!islocal) {
                            $.ajax({
                                url: gurl,
                                type: "POST",
                                ...
                                complete: function (data, Status) {
                                    if (Status != "success") {
                                        ...
                                        var status = $.evalJSON(data.responseText).d;
                            ...
                            $.each(fields, function (i) {
                                localdata[fields[i]] = coldata[i];
                            });
                            if (gpost.id == "new") {
                                var dataids = $($t).getDataIDs(), idx = 1;
                                if (dataids.length > 0) {
                                    idx = dataids[dataids.length - 1];
                                    idx++;
                                }
                                localdata["opstatus"] = "new";
                                Status = $($t).addRowData(idx, localdata) == true ? "success" : "failure";
                            }
                            else {
                                localdata["opstatus"] = "edited";
                                Status = $($t).setRowData(gpost.id, localdata) == true ? "success" : "failure";
                            }
                            if (Status != "success") {
                                ret[0] = false;
                                ret[1] = "Operation not successful";
                            } else {
                                // data is posted successful
                                // execute aftersubmit with the returned data from server
                                if ($.isFunction(rp_ge.afterSubmit)) {
        ...
        viewGridRow: function (rowid, p) {
            ...
            return this.each(function () {
                ...
                if ($("#" + IDs.themodal).html() != null) {
                    ...
                    var dh = isNaN(p.dataheight) ? p.dataheight : p.dataheight + "px";
                    var flr, frm = ($.browser.msie && parseInt($.browser.version) === 7) ? $("<div name='FormPost' id='" + frmgr + "' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:" + dh + ";'></div>") :
                              $("<form name='FormPost' id='" + frmgr + "' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:" + dh + ";'></form>"),
        ...
        delGridRow: function (rowids, p) {
            ...
            return this.each(function () {
                ...
                if ($("#" + IDs.themodal).html() != null) {
                    ...
                    // buttons at footer
                    tbl += "</tbody></table></div>"
                    var bS = "<a href='javascript:void(0)' id='dData' class='fm-button ui-primarytabclr ui-corner-all'>" + p.bSubmit + "</a>",
                    ...
                    $("#dData", "#" + dtbl + "_2").click(function (e) {
                        ...
                        if (ret[0] === false && !(islocal)) {
                            ...
                            if (!p.processing) {
                                ...
                                var postd = $.extend({ oper: "del", id: postdata, listname: $t.p.listname }, p.delData, onCS);
                                if (!(islocal)) {
                                    $.ajax({
                                        url: gurl,
                                        type: p.mtype,
                                    ...
                                    Status = $($t).delRowData(postd.id) == true ? "success" : "failure";
                                    if (Status != "success") {
                                        ret[0] = false;
                                        ret[1] = Status + " Status: " + data.statusText + " Error code: " + data.status;
                                    } else {
        ...
        navGrid: function (elem, o, pEdit, pAdd, pDel, pSearch, pView) {
            ...
            return this.each(function () {
                ...
                if (!$t.grid) { return; }
                if ($("#" + alertIDs.themodal + $t.p.id).html() == null) {
                    ...
                    createModal(alertIDs, "<div>" + o.alerttext + "</div><span tabindex='0'><span tabindex='-1' id='jqg_alrt'></span></span>", { gbox: "#gbox_" + $t.p.id, jqModal: true, drag: true, resize: false, caption: o.alertcap, top: vheight / 2 - 25, left: vwidth / 2 - 100, width: 200, height: 'auto', closeOnEscape: o.closeOnEscape }, "", "", true);
                }
                var tbd,
                    ...
                    $(tbd, navtbl)
                          ...
                          .click(function () {
                              ...
                              if (sr) {
                                  ...
                                  viewModal("#" + alertIDs.themodal, { gbox: "#gbox_" + $t.p.id, jqm: true, isalert: true });
                                  $("#" + alertIDs.themodal).focus();
                              }
                              return false;
                          }).hover(function () { $(this).addClass("ui-state-hover"); },
                              function () { $(this).removeClass("ui-state-hover"); }
                ...
                if (o.del) {
                    ...
                    $(tbd, navtbl)
                          ...
                          .click(function () {
                              ...
                              else { viewModal("#" + alertIDs.themodal, { gbox: "#gbox_" + $t.p.id, jqm: true, isalert: true }); $("#jqg_alrt").focus(); }
                              $("#" + alertIDs.themodal).focus();
                              return false;
                          }).hover(function () { $(this).addClass("ui-state-hover"); },
                              function () { $(this).removeClass("ui-state-hover"); }
