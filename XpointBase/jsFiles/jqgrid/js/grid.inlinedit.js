  ...
  **/
  $.fn.extend({
    ...
    saveRow: function(rowid, succesfunc, url, extraparam, aftersavefunc, errorfunc, afterrestorefunc) {
      return this.each(function() {
        var $t = this, nm, tmp = {}, tmp2 = {}, editable, fr, cv, ind, count = 0;
        if (!$t.grid) { return; }
        ind = $($t).getInd(rowid, true);
        if (ind == false) { return; }
        ...
        if (editable === "1" && url) {
              ...
              if ($t.p.colModel[i].hidden === true) { tmp[nm] = $(this).html(); }
                ...
                switch ($t.p.colModel[i].edittype) {
                  case "checkbox":
                    ...
                    break;
                    case  'singleperson':
                      tmp[nm] = !$t.p.autoencode ?$($("input, textarea", this).get(0)).val() : $.jgrid.htmlEncode($($("input, textarea", this).get(0)).val());
                    break;
                  case 'text':
                  case 'password':
                  case 'textarea':
          ...
          if (!$t.grid.hDiv.loading) {
            ...
            if (url == 'clientArray') {
              ...
              var coldata = new Array(count);
              var fields = new Array(count);
              var idx = 0;
              $("td", $t.rows[ind]).each(function(i) {
                /*check if the colModel has a value at the given index*/
                if ($t.p.colModel[i] != null) {
                  nm = $t.p.colModel[i].name;
                  /*check if the colmodel  referenced is part of fields to be edited*/
                  if (tmp[nm] != null) {
                    coldata[idx] = tmp[nm];
                    fields[idx] = nm;
                    idx++;
              ...
              $.ajax({ url: url,
                ...
                complete: function(res, stat) {
                  if (stat === "success") {
                    var ret;
                    if ($.isFunction(succesfunc)) { ret = true; }
                    else ret = true;
                    if (ret === true) {
