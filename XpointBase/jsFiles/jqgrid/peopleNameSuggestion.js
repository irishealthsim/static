             ...
             if (vals != null) {
                 //$(vals).each(function () {
                     ...
                     if (val) {
                         ...
                         pplAttr.push("{ FormattedVal:'" + val.FormattedVal + "', Name:'" + vals + "'}");
                     }
                 //});
...
function initPeopleNameSuggest(el, vl, suggestOpts) {
    var prefillVls ;//= [{ Name: "", FormattedVal: "" }];
    if (vl) {
        //prefillVls = eval("(" + vl + ")");
        prefillVls = [{ Name: vl, FormattedVal: vl }];
    }
    var elJ = $(el);
    elJ.val('');
    var elId = elJ.attr('id');
    suggestOpts = $.extend({
        startText: '', asHtmlID: elId, singleEntry: false, anonymousEntry: true, url: "/_vti_bin/xpointbase/ProfileService.svc/Users",
        ...
        selectedValuesProp: "Name", retrieveComplete: function (data) {
