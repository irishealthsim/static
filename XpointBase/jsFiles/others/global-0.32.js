...
//{{{ messy highlighting stuff
	...
	if (f == '/documentation' || f == '/tutorials') f = '/demos';
	...
	*/		
	...
	//{{{ plugins
		...
		var player = $f("player2", v.core, {
			clip:  {
				scaling:'scale',
				url: 'http://static.flowplayer.org/video/flowplayer-700.flv',
	...
	$f("player3", {src: v.core, bgcolor: '#112233'}, {
			...
			clip: {
				...
				url: flashembed.isSupported([9, 115]) ? null : 'http://e1p1.simplecdn.net/flowplayer//flowplayer-700.flv'
			},
		...
		*/ 
		$("a[rel]").overlay({
			...
			onBeforeLoad: function() {			
				this.getBackgroundImage().expose("#123").load();
			}, 
	...
	$("#latestPosts h2").click(function() {
		...
		posts.slideToggle(function()  {
			setCookie("postsDisplay", posts.css("display"), 7);
