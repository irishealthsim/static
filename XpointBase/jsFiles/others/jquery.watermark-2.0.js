...
(function ($) {
    var old_ie = false; // $.browser.msie && $.browser.version < 8; // this not working with new jQeury.

    // check if its IE7 or old.
    // modified by Ibrahim As'ad
...
.bind('blur.jq_watermark change.jq_watermark', function () {
  if (!$.watermarker.checkVal($(this).val(), watermark_label))
  	watermark_label.stop().fadeTo(options.animDuration, .99); 
})
